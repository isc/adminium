...
class TextEdition {
  ...
  enableRichTextEditor(){
    ...
    fullTagList.forEach((tag) => {
      parserRules.tags[tag] = {check_attributes: {}}
    })
    parserRules.tags['a'].check_attributes = { href: 'url', target: 'alt' }
    $('form.resource-form textarea').wysihtml5({ html: true, parserRules: parserRules, toolbar: {fa: true} })
  }
  ...
  addSwitchingLinks(){
    ...
    const text = this.pref === 'rich_text_editor' ? '<i class="fa fa-exchange"></i> Plain text area' : '<i class="fa fa-exchange"></i> Rich text editor'
    $('form.resource-form textarea').each((_, textarea) =>
      $(textarea).parent().append($('<a />').html(text).attr('href', '#').addClass('text-edition-switch'))
    )
  }

  switchEditionPreference(){
    this.pref = this.pref === 'rich_text_editor' ? 'plain_text_area' : 'rich_text_editor'
